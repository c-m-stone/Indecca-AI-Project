...
export const useChatMessages = (notebookId?: string) => {
  ...
  const sendMessage = useMutation({
    mutationFn: async (messageData: {
      ...
      const webhookResponse = await supabase.functions.invoke('send-chat-message', {
        body: {
          ...
          user_id: user.email,
          chat_mode: messageData.chatMode
        }
      });
    ...
    onSuccess: () => {
      ...
      // Update the notebook's updated_at timestamp to move it to top of list
      if (notebookId) {
        ...
        supabase
          ...
          .then(({ error }) => {
            if (error) {
              console.error('Error updating notebook timestamp:', error);
            } else {
              console.log('Notebook timestamp updated successfully');
              // Invalidate notebooks query again after timestamp update to refresh the ordering
              queryClient.invalidateQueries({ queryKey: ['notebooks'] });
            }
          });
      }
