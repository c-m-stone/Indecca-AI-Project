...
const AddSourcesDialog = ({
  ...
  const processFileAsync = async (file: File, sourceId: string, notebookId: string) => {
    try {
      ...
      // Improved file type detection based on extension and MIME type
      const getFileType = (file: File): 'pdf' | 'text' | 'audio' => {
        const extension = file.name.split('.').pop()?.toLowerCase() || '';
        const mimeType = file.type.toLowerCase();
        
        // Audio files
        const audioExtensions = ['mp3', 'mp4', 'mpeg', 'mpga', 'm4a', 'wav', 'webm'];
        if (audioExtensions.includes(extension) || mimeType.startsWith('audio/') || mimeType.startsWith('video/')) {
          return 'audio';
        }
        ...
        // Everything else as text (documents, spreadsheets, images, etc.)
        return 'text';
      };
      ...
      console.log('Document processing initiated for:', sourceId);
            ...
            >
              <div className="flex flex-col items-center space-y-4">
                ...
                <p className="text-xs text-gray-500">
                  Supported file types: Documents (PDF, DOC, DOCX, PPT, PPTX, TXT, etc.), Images (JPG, PNG, GIF, SVG, etc.), Audio/Video (MP3, MP4, WAV, etc.), Web files (HTML, XML), Spreadsheets (XLSX, XLS, CSV, ODS, etc.), and many more legacy formats.
                </p>
                <input
                  id="file-upload"
                  type="file"
