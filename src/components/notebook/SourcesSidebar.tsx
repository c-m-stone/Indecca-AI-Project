...
import { Plus, MoreVertical, Trash2, Edit, Loader2, CheckCircle, XCircle, Upload, ArrowLeft } from 'lucide-react';
...
const SourcesSidebar = ({
  ...
  const handleBackToAttachments = () => {
    console.log('SourcesSidebar: Back to attachments clicked');
    setSelectedSourceForViewing(null);
    onCitationClose?.();
  };
      ...
      <div className="w-full flex flex-col h-full overflow-hidden">
        <div className="p-4 border-b border-gray-200 flex-shrink-0">
          <div className="flex items-center space-x-3">
            <Button variant="ghost" onClick={handleBackToAttachments} className="p-2">
            ...
            <div className="flex items-center space-x-2">
              <div className="w-6 h-6 bg-white rounded border border-gray-200 flex items-center justify-center flex-shrink-0 overflow-hidden">
                {(() => {
                  ...
                  const iconUrl = iconMap[displayCitation.source_type] || iconMap['text'];
                  return (
                    <img 
                      src={iconUrl} 
                      alt={`${displayCitation.source_type} icon`} 
                      className="w-full h-full object-contain"
                      onError={(e) => {
                        const target = e.target as HTMLImageElement;
                        target.style.display = 'none';
              ...
              <span className="font-medium text-gray-900 truncate">{displayCitation.source_title}</span>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="w-full flex flex-col h-full overflow-hidden">
      <div className="p-4 flex-shrink-0">
        <Button variant="outline" size="sm" className="w-full" onClick={() => setShowAddSourcesDialog(true)}>
        ...
        <div className="p-4">
          {isLoading ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-sm text-gray-600 mb-4">Click Add above to add PDFs, text, or audio files.</p>
            </div>
          )}
        </div>
      </ScrollArea>
