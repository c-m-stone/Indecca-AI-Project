...
const SourceContentViewer = ({ 
  ...
  // Check if summary is long enough to need scrolling/popout
  const isLongSummary = sourceSummary && sourceSummary.length > 300;
  return (
        ...
        <div className="flex-shrink-0">
          <Accordion type="single" value={accordionValue} onValueChange={setAccordionValue} collapsible>
            <AccordionItem value="guide" className="border-0">
              <AccordionTrigger 
                className="px-4 py-3 text-sm font-medium hover:no-underline hover:bg-blue-50 border-b border-gray-200" 
                style={{ color: '#234776' }}
                chevronColor="#234776"
              >
              ...
              <AccordionContent className="px-4 pb-4 border-b border-gray-200">
                <div className="text-sm text-gray-700 space-y-4">
                  <div>
                    <h4 className="font-medium mb-2">Summary</h4>
                    {isLongSummary ? (
                      <ScrollArea className="max-h-20 pr-2">
                        <p className="leading-relaxed text-sm">{sourceSummary}</p>
                      </ScrollArea>
                    ) : (
                          ...
                          <DialogContent className="max-w-2xl max-h-[80vh]">
                            <DialogHeader>
                              <DialogTitle className="flex items-center space-x-2">
                                <div className="w-5 h-5 bg-white rounded border border-gray-200 flex items-center justify-center flex-shrink-0 overflow-hidden">
                                  {getSourceIcon(citation.source_type)}
                                </div>
                                <span className="truncate">{citation.source_title}</span>
                              </DialogTitle>
                            </DialogHeader>
